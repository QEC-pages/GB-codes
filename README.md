...
- The file `gb-verify.gap` contains the list of all codes (circulant length, distance, and two lists of degrees of the non-zero monomials) and a GAP program to verify the distance or write the files.
- The file `gb-verify_gap.out.gz` is the stdout generated by running `gap.sh gb-verify.gap`.  
